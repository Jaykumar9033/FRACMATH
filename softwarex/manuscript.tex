...
\graphicspath{{./}{paper_figs/}{figures/}{../paper_figs/}}

% =====================================================================
% Empty figure placeholder, kept for benchmarks 2 and 3 until their
% figures are ready. Usage: \emptyfig{width fraction}{aspect}{label}
% =====================================================================
\newcommand{\emptyfig}[3]{%
  \begin{tikzpicture}
    \draw[dashed, line width=0.6pt, color=gray!70]
      (0,0) rectangle (#1\linewidth, #2\linewidth);
    \node[align=center, font=\small\itshape, color=gray!90,
          text width={#1\linewidth-2em}]
      at ({#1\linewidth/2}, {#2\linewidth/2}) {#3};
  \end{tikzpicture}%
}
...
\title{FRACMAT: A vectorized MATLAB framework for continuum damage
mechanics with crack band regularization}

\author[unm]{Jaykumar~Mavani}
\author[unm]{Madura~Pathirage\corref{cor1}}
\ead{TODO: add corresponding author email}
\cortext[cor1]{Corresponding author}
\address[unm]{Department of Civil, Construction, and Environmental
Engineering, University of New Mexico, Albuquerque, NM, USA}

\begin{abstract}
\texttt{FRACMAT} is an open-source MATLAB framework for finite element
simulation of fracture in quasibrittle materials. The software implements
scalar isotropic continuum damage mechanics, exponential softening, and
crack-band regularization with a direction-dependent Oliver characteristic
length. It provides a vectorized MATLAB solver, benchmark data, plotting
scripts, and an Abaqus UMAT comparison workflow. The principal validation
case is a notched two-dimensional three-point bending benchmark checked
against an Oliver-matched Abaqus/Standard UMAT; additional three-dimensional
Nooru-Mohamed and Brokenshire torsion examples demonstrate mixed-mode and
torsion-driven crack paths.
\end{abstract}
...
\url{https://github.com/Jaykumar9033/FRACMATH} \\
\hline
...
GitHub repository \\
\hline
C8 & Support email for questions & TODO: add corresponding author support email \\
\hline
\end{tabular}
...
vectorized modified Newton Raphson scheme that uses a secant tangent and
refactors the free-degree-of-freedom stiffness block once per load step,
all under displacement control.

The package comes with three validated cases. The main benchmark is a
...
validation cases run in MATLAB only: a 3D mixed mode Nooru-Mohamed test
and a 3D notched beam torsion test.

Source code, documentation, and benchmark inputs live at
\url{https://github.com/Jaykumar9033/FRACMATH} under the MIT license. 
% TODO before submission: replace XXXXXXX with the real Zenodo DOI.
...
\citep{abaqus} and the C++ research code OOFEM \citep{oofem}. Both work
well. Both also take real effort to learn for graduate students and
engineers who do not already write Fortran or modern C++. Adapting either
...
\texttt{FRACMAT} fills that gap. The reader we have in mind is a graduate
...
The equivalent strain follows the modified von Mises measure of
\citet{deVree},
\begin{equation}
...
each material point. Therefore, the updated Abaqus comparison uses the
same direction-dependent Oliver bandwidth formula as the MATLAB solver,
...
mesh, and boundary conditions are shown in Figure~\ref{fig:b1-mesh}. The
...
For the updated timing comparison, MATLAB R2024a was run in single-thread
...
% TODO before submission: replace the bracket below with your real
%       machine: operating system, CPU (cores / clock), RAM, and storage.
[\textit{operating system, CPU (cores and clock), RAM, and storage to be
specified}].

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{fig_mesh.png}
  \caption{Notched 3PB specimen ($D=100$~mm): geometry, the refined CPS3
    mesh in the notch zone, and the support and load layout.}
  \label{fig:b1-mesh}
\end{figure}

The simulated MATLAB peak load is 3.63982~kN (3639.82~N) at a CMOD of
0.022811~mm. The Abaqus UMAT peak load is 3.60913~kN (3609.13~N) at a
CMOD of 0.022485~mm. The full load--CMOD response is shown in
Figure~\ref{fig:b1-loadcmod}. The MATLAB and Abaqus curves agree closely
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\linewidth]{load_cmod_comparison.png}
  \caption{Load--CMOD response for the 2D 3PB case. The MATLAB and
    Oliver-matched Abaqus UMAT curves track each other closely up to the
    peak. The post-peak branches remain close but are not bitwise
    identical because the two solvers use different nonlinear solution
    and state-update paths.}
  \label{fig:b1-loadcmod}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{fig_damage_peak.png}\\[0.6em]
  \includegraphics[width=0.85\linewidth]{fig_damage_postpeak.png}
  \caption{MATLAB fully damaged band ($\omega \ge 0.99$) at peak load
    (top, 3.64~kN, 17 elements) and post-peak (bottom, 0.13~kN,
    CMOD~$\approx$~0.30~mm, 118 elements). The band grows upward from the
    notch tip.}
  \label{fig:b1-damage}
\end{figure}

For an Oliver-matched check on the crack location and width, the same
case was also run in Abaqus/Standard using the updated UMAT. The Abaqus
damage variable is stored as SDV2 and is plotted on the undeformed mesh
in Figure~\ref{fig:b1-abqdamage}. The black band corresponds to elements
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{abaqus_fig_damage_last_step.png}\\[0.6em]
  \caption{Abaqus damage field (SDV2) on the undeformed mesh from the
    Oliver-matched UMAT run. The black band marks the damaged crack path
    extracted with the same threshold used for the MATLAB damage figures,
    $\omega \ge 0.99$.}
...
Stiffness assembly takes most of the run at about 82.5 percent, the linear
solve about 7.9 percent, the damage update about 2.1 percent, and the rest
goes to bookkeeping and output. At this problem size the sparse direct
solve is cheap, so the per-element constitutive update, crack-band length
calculation, and sparse assembly dominate the MATLAB wall-clock cost.

The Abaqus comparison uses the same geometry, mesh, material constants,
loading, boundary conditions, scalar CDM law, modified von Mises
equivalent strain, exponential softening law, and crack-band energy
scaling as the MATLAB solver. In the updated Abaqus workflow, the UMAT no
longer uses the Abaqus characteristic element length \texttt{CELENT} as
the crack-band length. Instead, the preprocessing script writes the T3
shape-function gradients to \texttt{oliver\_t3\_gradN.dat}, and the UMAT
computes Oliver's direction-dependent projected bandwidth from the current
maximum principal strain direction using Eq.~\eqref{eq:oliver-t3}.
Therefore, the Abaqus run is described as an Oliver-matched UMAT
comparison, not as a \texttt{CELENT} crack-band comparison.

The two implementations should still not be described as bitwise
identical. MATLAB and Abaqus use different global solution procedures,
increment control, convergence checks, floating-point operation order, and
state-variable update paths. Therefore, small differences may remain in
the peak load, post-peak branch, and residual tail even when the same
Oliver bandwidth formula is used.

Figure~\ref{fig:b1-time} compares the wall-clock cost of the two runs.
Using the measured wall-clock values in Table~\ref{tab:abqcompare},
...
\midrule
Peak load (kN)       & 3.63982 & 3.60913 & 1.009 \\
...
\vspace{0.4em}
\footnotesize{These values correspond to the updated Oliver-matched UMAT
comparison used in the current SoftwareX draft.}
\end{table}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{nooru_mesh_3D.png}
    \caption{3D mesh.}
    \label{fig:b2-nooru-mesh-3d}
  \end{subfigure}

  \caption{Nooru-Mohamed benchmark: 2D boundary conditions and 3D mesh.}
  \label{fig:b2-mesh}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{nooru_damage_evolution_3x3.png}
  \caption{Damage evolution $\omega$ for the Nooru-Mohamed run, from
  first localization at the notches (inc 29) to coalescence (inc 900).
  Only $\omega \ge 0.855$ is shaded.}
  \label{fig:b2-damage-evolution}
\end{figure}
% TODO: add a results table (peak load, comparison with Nooru-Mohamed
%       experimental data) before submission.
...
boundary grips are shown in Figure~\ref{fig:b2-mesh}. The solver is the
...
Figure~\ref{fig:b2-damage-evolution} traces the predicted response from
the first localization at the notch tips (increment~29) through to full
coalescence of the two bands (increment~900); only elements with
$\omega \ge 0.95$ are shaded, so the crack path itself stands out from
the diffuse process zone around it. The two curved bands and their final
separation reproduce the experimentally observed mixed mode crack
pattern, which confirms that the scalar damage model with crack band
regularization carries over from the planar mode I case to a fully three
dimensional mixed mode setting without any change to the constitutive
routine.
...
Brokenshire's torsion test \citep{jefferson_torsion} uses a
...
remaining top corner (Figure~\ref{fig:b3-mesh}), so the beam is twisted
and the plane of maximum principal tension turns in space around the
notch front, lining up with no face of the mesh. This is exactly the
situation in which a characteristic length tied to a fixed mesh direction
would lose objectivity. We avoid that by using Oliver's projected
characteristic length, recomputed from the local principal direction at
each integration point, so the dissipated energy stays close to $G_F$
however the crack plane is oriented relative to the mesh.
...
the notch gives the crack opening (Figure~\ref{fig:b3-mesh}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\linewidth]{torsion.png}
  
  \caption{Brokenshire torsion specimen (mm). Three supports (blue) and
    one load (red); A and B are the two monitored points across the
    notch.}
  \label{fig:b3-mesh}
\end{figure}
...
% =====================================================================
% Torsion damage evolution montage: 8 live snapshots, 3 per row.
% Drop the 8 PNGs on the graphicspath (./ or paper_figs/).
% =====================================================================
\newcommand{\torsnap}[1]{%
...
\begin{figure}[htbp]
  ...
  \caption{Damage evolution $\omega$ over the torsion run, labelled by
    twist $\theta$ and increment. The band nucleates at the notch front
    and spirals toward the loaded corner.}
  \label{fig:b3-damage-evolution}
\end{figure}

% TODO: add a results table (peak torque, load and displacement at A and
%       B, comparison with Brokenshire experimental data) before submission.

% =====================================================================
\section*{Impact}
% =====================================================================

\texttt{FRACMAT} is intended to make continuum damage mechanics and
...
\texttt{FRACMAT} provides a compact MATLAB implementation of scalar CDM
...
% TODO: add funding sources, lab support, and any acknowledgements here
%       before SoftwareX submission.
[Funding sources and support to be added before submission.]
