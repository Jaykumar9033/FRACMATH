...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Notched 2D three-point bending benchmark. The MATLAB and
    Oliver-matched Abaqus UMAT curves track each other closely up to the
    peak, while the post-peak branches remain close but not bitwise
    identical because the solvers use different nonlinear solution and
    state-update paths.}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.86\linewidth}
    \centering
    \includegraphics[width=\linewidth]{fig_damage_peak.png}
    \caption{MATLAB peak load: 3.64~kN, 17 elements.}
  \end{subfigure}

  \vspace{0.45em}
  \begin{subfigure}{0.86\linewidth}
    \centering
    \includegraphics[width=\linewidth]{fig_damage_postpeak.png}
    \caption{MATLAB post-peak: 0.13~kN, CMOD~$\approx$~0.30~mm,
      118 elements.}
  \end{subfigure}
  ...
  \begin{subfigure}{0.86\linewidth}
    ...
    \caption{Abaqus SDV2 damage field from the Oliver-matched UMAT run.}
  \end{subfigure}
  \caption{Fully damaged bands for the 2D 3PB benchmark. The MATLAB band
    grows upward from the notch tip; the Abaqus panel marks the damaged
    crack path extracted with the same threshold, $\omega \ge 0.99$.}
  \label{fig:b1-damage}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.95\linewidth}
    ...
    \caption{Damage evolution from first localization at the notches
      (inc 29) to coalescence (inc 900).}
    \label{fig:b2-nooru-damage}
  \end{subfigure}

  \caption{Nooru-Mohamed benchmark: boundary conditions, 3D mesh, and
    damage evolution $\omega$. Only $\omega \ge 0.855$ is shaded in the
    damage-evolution panels.}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.62\linewidth}
    \centering
    \includegraphics[width=\linewidth]{torsion.png}
  ...
  \caption{Brokenshire torsion benchmark and damage evolution
    $\omega$ over the imposed twist history. The specimen panel shows
    the three supports (blue), one load (red), and monitored points A and
    B; the damage panels are labelled by twist $\theta$ and increment.
    The band nucleates at the notch front and spirals toward the loaded
    corner.}
