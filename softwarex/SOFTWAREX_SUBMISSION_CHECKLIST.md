# SoftwareX Submission Checklist

This repository is a local source package for the SoftwareX manuscript. It does
not submit anything automatically.

## Manuscript Source Package

- `manuscript.tex`
- `paper.bib`
- `paper_figs/`
- `figures/`

## Required Before Online Submission

- Replace `TODO: add corresponding author email` in `manuscript.tex`.
- Confirm the GitHub repository URL in the metadata table.
- Add the final Zenodo or repository release DOI when available.
- Replace the acknowledgement placeholder with the final funding/support text.
- Compile the LaTeX source and upload the generated PDF plus a source zip in the
  SoftwareX submission system.

## Required In The GitHub Code Repository

- Public GitHub repository.
- `README.md` with installation, examples, and test instructions.
- `LICENSE.txt` or `Licence.txt`.
- Version tag or release matching the manuscript metadata.
- `CITATION.cff`.
- Reproducibility instructions and expected benchmark outputs.

## Current SoftwareX Metadata Values

- Code version: `v1.0.0`
- License: MIT
- Version control: git
- Languages/tools: MATLAB, Fortran UMAT, Abaqus/Standard, Python plotting
  scripts
