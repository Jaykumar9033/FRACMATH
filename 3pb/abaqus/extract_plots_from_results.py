...
def write_omega_csv(frame, path, threshold=DEFAULT_THRESHOLD):
    with open(path, "w") as handle:
        ...
        count, max_omega, cracked = 0, -1.0e99, 0
        for value in field.values:
            ...
            if omega >= threshold:
                cracked += 1
    return count, max_omega if max_omega > -1.0e98 else -1.0, cracked
...
def extract_from_odb(odb_path, threshold=DEFAULT_THRESHOLD):
    ...
    try:
        ...
        for frame_idx, name in (
            ...
            (postpeak_frame, "omega_fully_cracked.csv"),
            (final_frame, "omega_last.csv"),
        ):
            frame = step.frames[frame_idx] if frame_idx is not None else None
            count, max_omega, cracked = write_omega_csv(frame,
                                                        os.path.join(plotdata_dir, name),
                                                        threshold)
            frame_time = float(frame.frameValue) if frame is not None else -1.0
            snapshot_rows.append((name, frame_idx, frame_time, count, max_omega, cracked))
        ...
        with open(diagnostic, "w") as handle:
            ...
            for name, frame_idx, frame_time, count, max_omega, cracked in snapshot_rows:
                handle.write("  %-24s frame=%s time=%.9f rows=%d max_omega=%.6f cracked=%d\n" %
                             (name, str(frame_idx), frame_time, count, max_omega, cracked))
