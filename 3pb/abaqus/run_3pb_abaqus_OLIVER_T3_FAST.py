...
FIELD_VARS = ('SDV',)
FIELD_FREQ = int(os.environ.get('ABQ_FIELD_FREQ', '1'))

AUTO_PLOT     = (os.environ.get('ABQ_AUTO_PLOT', '0') == '1')

PYTHON_EXE    = r''
POSTPEAK_CMOD = float(os.environ.get('ABQ_POSTPEAK_CMOD', '0.3'))
FULL_DAMAGE_THRESHOLD = float(os.environ.get('ABQ_DAMAGE_THRESHOLD', '0.99'))
...
def build_model(case_dir, umat_file):
    ...
    # SDV1=kappa, SDV2=omega, SDV3=Oliver h, SDV4=Oliver-table flag,
    # SDV5=fully cracked mask (omega >= 0.99 in the UMAT, matching MATLAB).
    mat.Depvar(n=5)
...
def frame_has_damage(frame):
    fld, _, _ = get_damage_field(frame)
    if fld is None:
        return False
    try:
        return len(fld.values) > 0
    except Exception:
        return False
...
def closest_damage_frame_index(step, target_time, prefer_at_or_after=False):
    candidates = []
    for i, fr in enumerate(step.frames):
        if not frame_has_damage(fr):
            continue
        t = float(fr.frameValue)
        if prefer_at_or_after and t < float(target_time):
            continue
        candidates.append((abs(t - float(target_time)), i))
    if not candidates and prefer_at_or_after:
        return closest_damage_frame_index(step, target_time, False)
    if not candidates:
        return closest_frame_index(step, target_time)
    candidates.sort()
    return candidates[0][1]

def last_damage_frame_index(step):
    for i in range(len(step.frames) - 1, -1, -1):
        if frame_has_damage(step.frames[i]):
            return i
...
def write_omega_csv(frame, path):
    ...
    with open(path, 'w') as f:
        ...
        for v in fld.values:
            ...
            if w > mx:
                mx = w
            if w >= FULL_DAMAGE_THRESHOLD:
                ncrack += 1
    return n, (mx if mx > -1.0e98 else -1.0), ncrack
...
def dump_plotdata(odb, step, res_dir, postpeak_idx, last_idx):
    ...
    snapshots = []
    for idx, name in ((postpeak_idx, 'omega_postpeak.csv'),
                      (postpeak_idx, 'omega_fully_cracked.csv'),
                      (last_idx, 'omega_last.csv')):
        fr = frame_at(idx)
        n, mx, ncrack = write_omega_csv(fr, os.path.join(pd_dir, name))
        frame_time = float(fr.frameValue) if fr is not None else -1.0
        snapshots.append((name, idx, frame_time, n, mx, ncrack))
    ...
    try:
        ...
        for i, (c, r) in enumerate(rows):
            if c >= POSTPEAK_CMOD:
                ...
                pp_row = i
                break
        pp_frame = last_damage_frame_index(step) if pp_time is None \
            else closest_damage_frame_index(step, pp_time, prefer_at_or_after=True)

        last_frame = last_damage_frame_index(step)

        snapshot_rows = dump_plotdata(odb, step, res_dir,
                                      pp_frame, last_frame)
        ...
        with open(os.path.join(res_dir, 'abaqus_timing.txt'), 'w') as f:
            ...
            f.write('Post-peak/fully cracked CMOD target: %.6f mm\n' % POSTPEAK_CMOD)
            f.write('Post-peak/fully cracked load: %.2f N\n' % rows[pp_row][1])
            f.write('Rows:       %d\n' % len(rows))
            ...
            f.write('\nDamage snapshots from SDV2 omega:\n')
            f.write('  threshold: omega >= %.2f\n' % FULL_DAMAGE_THRESHOLD)
            for name, frame_idx, frame_time, n, mx, ncrack in snapshot_rows:
                f.write('  %-24s frame=%s time=%.9f rows=%d max_omega=%.6f cracked=%d\n'
                        % (name, str(frame_idx), frame_time, n, mx, ncrack))
...
def _load_plotdata(np, pd_dir):
    ...
    w_full = omega('omega_fully_cracked.csv')
    if not w_full.any():
        w_full = omega('omega_postpeak.csv')
...
def plot_main(res_dir):
    ...
    _fig_damage(np, plt, PolyCollection, cmap, D['nodes'], D['elems'], D['w_full'],
                'Fully cracked: %.2f kN' % (pp_load / 1000.0),
                os.path.join(res_dir, 'abaqus_fig_damage_fully_cracked'))
    ...
    for nm in ('abaqus_load_cmod_fig',
               ...
               'abaqus_fig_damage_fully_cracked',
