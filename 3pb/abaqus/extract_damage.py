...
def try_plot(pd_dir, out_dir, thresh):
    ...
    one_fig('omega_fully_cracked.csv', 'fully cracked', os.path.join(out_dir, 'abaqus_fig_damage_fully_cracked'))
...
def main():
    ...
    diag.append('Frame indices -> postpeak: %s  last: %s'
                % (str(pp_i), str(last_i)))

    for idx, nm in ((pp_i, 'omega_postpeak.csv'),
                    (pp_i, 'omega_fully_cracked.csv'),
